...
export const ZH = {
  ...
  't3-h': '不追踪使用情况；你不打开的通路，什么都不会出去',
  't3-p': 'Spool 没有分析统计、遥测或崩溃上报，主程序本身也发不出网络请求——它里面根本没装 HTTP 客户端。内容只会顺着你自己打开的那条路离开这台电脑：把 Pack 粘贴到某个服务、允许 MCP 客户端读取、运行命令行 AI 的起草跟进目标、“跟进”和“周回顾”，或者打开 API 引擎让 Spool 替你压缩项目。前三条由接收内容的程序决定后续；API 引擎这一条，请求由一个单独的随包小程序发出，用你自己的 key 和额度。剪贴板只在触发捕捉时读取一次，不会在后台持续监控。',
  ...
  a10: '日常用不需要。AI 客户端读取项目时，使用的是你在该客户端已有的账号与额度；命令行 AI 执行“跟进”等动作时，也使用对应 CLI 已登录账号的额度。有一个可选功能确实要 key：<b>API 引擎</b>，它负责压缩项目和过期检测。它默认是关的，要你自己打开、自己填 key，每跑一次花的是你自己的额度。',
  q11: '必须连接 AI 才能使用吗？',
  ...
  'fb-k': '用过一阵之后',
  'fb-h': '告诉我用得怎么样。',
  'fb-p': 'Spool 不需要账号，也没有任何遥测——你怎么用它，没有一个字会传到我这里，这也意味着它没法替你告诉我任何事。好用也好，坏了也好，你不说我就不知道。',
  ...
  'story-arch-svg-aria': 'Spool 架构：图形界面和捕捉浮窗在本机连到 Rust 与 Tauri 内核以及一个 SQLite 数据库。spool --mcp 模式通过 stdio 与外部 MCP 客户端通信。内核会把 Claude Code、Codex CLI 或 Gemini CLI 作为子进程启动；打开 API 引擎之后，还会启动随包的 spool-ai 子进程。离开这台机器的网络路径一共三条，分别来自这两个子进程和那个 MCP 客户端；Spool 主进程自己不发出任何 HTTP 请求。',
  'story-arch-svg-title': 'Spool 的本地架构，以及三条对外的路径',
  'story-arch-svg-desc': 'Spool 进程里装着桌面图形界面、捕捉浮窗、Rust 与 Tauri 内核、spool --mcp 的 stdio 服务器，以及一个 SQLite 数据库。外部 MCP 客户端通过 stdio 连进来。内核可以把 Claude Code、Codex CLI 或 Gemini CLI 作为独立子进程启动；打开 API 引擎之后，还会启动随包的 spool-ai 子进程。这三个程序分别通过三条不同的网络路径联系各自的服务商。没有任何一条箭头从 Spool 主进程自身指向网络。',
  ...
  'story-arch-api': 'API 引擎子进程',
  'story-arch-api-sub': '默认关闭 · 用你自己的 key',
  'story-arch-api-mono': '整个 app 里唯一开 socket 的',
  ...
  'story-arch-provider-3': '你自己填的那个端点',
  'story-arch-provider-3-sub': '网络路径 3 · 用你自己的 key',
  'story-arch-flow-local': '本机',
  'story-arch-flow-spawn': '启动 + stdio',
  'story-arch-path-1': '网络路径 1',
  'story-arch-path-2': '网络路径 2',
  'story-arch-path-3': '网络路径 3',
  'story-arch-no-http': 'Spool 主进程自己不发出任何 HTTP 请求。',
  'story-arch-boundary': '网络边界 —— 只有那三条高亮路径会穿过它',
  'story-cap-arch': '这条网络边界是字面意义上的，而且它画在进程的边上，不是一句承诺：外部 MCP 客户端、你自己登录过的命令行，以及打开 API 引擎之后随包启动的 <code>spool-ai</code> 子进程，各自去联系一家服务商。Spool 主进程自己不发 HTTP 请求，也不监听任何网络端口——它里面根本没编进 HTTP 客户端。三条路里只有 API 引擎那一条要拿 key，它默认是关的，而且 key 存在系统钥匙串里，不在 Spool 的设置文件里。',
  'story-arch-p2': '图形界面和捕捉浮窗在本机与 Rust/Tauri 内核通信。内核拥有唯一的那个 SQLite 数据库。运行 <code>spool --mcp</code> 会把同一个库通过 stdio 交给 AI 客户端读取；命令行引擎位走的是另一条路——把 Claude Code、Codex CLI 或 Gemini CLI 作为独立进程启动。Gemini 支持“周回顾”和起草跟进目标，但不支持“跟进”。API 引擎是第三条，也是最新的一条：它启动 <code>spool-ai</code>，一个装在包里的小程序，从 stdin 收一个 JSON 请求，发一次 HTTPS，再把一个 JSON 信封写回来。它不接受明文 <code>http://</code>，而 key 只从 stdin 进去——绝不走命令行，那里同机器上任何程序都读得到。三条路保住的是同一个区分：哪些是 Spool 在本机做的事，哪些是另一个进程在联网做的事。',
