...
/* The sample Pack (WORKPLAN-2026-08-20 §2.4). A Pack is text, so "try it without
   installing" needs no backend: hand over the real file and let the reader's own AI answer
   from it. The file itself is generated by scripts/build-sample-pack.ts through the same
   assemble() the app runs — never written by hand, or it drifts from the product.

   The Chinese page gets the Chinese Pack. The href in the HTML is the English one because
   build-site-zh.mjs bakes both pages from that single source and only rewrites `assets/`
   into `../assets/`; picking the language here keeps that rewrite the only path rule. */
(function () {
  var btn = document.getElementById('copy-sample-pack');
  var link = document.getElementById('open-sample-pack');
  if (!btn || !link) return;
  if ((document.documentElement.lang || '').slice(0, 2) === 'zh') {
    link.href = link.getAttribute('href').replace('sample-pack.md', 'sample-pack.zh.md');
  }
  ...
  btn.addEventListener('click', function () {
    fetch(link.href)
      ...
      .then(function () {
        var done = document.getElementById('copy-sample-pack-done');
        btn.textContent = (done && done.textContent) || idle;
        setTimeout(function () {
          btn.textContent = idle;
        }, 4000);
      })
      /* Clipboard access can be refused (permissions, an insecure origin, an older
         browser). Falling back to the file itself is better than a dead button: the
         reader can still select all and copy by hand. */
      .catch(function () {
