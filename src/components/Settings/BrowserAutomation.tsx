...
const statusBadge = (s: Status): { label: string; color: string } => {
  switch (s) {
    case 'granted':
      return { label: tt('✓ 已授权'), color: 'var(--status-active)' };
    case 'denied':
      return { label: tt('✗ 未授权'), color: 'var(--urgent)' };
    case 'untested':
    default:
      return { label: tt('⚪ 未测试'), color: 'var(--muted)' };
  }
};
  ...
  return (
    <div>
      <p className="mb-2 text-xs text-muted">
        {t('首次从浏览器捕捉时，macOS 会请求"自动化"权限。允许后，捕捉的来源会显示标签页标题；否则只会显示浏览器名。点击"测试"可重新触发该提示。')}
      </p>
