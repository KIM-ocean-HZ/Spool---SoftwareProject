    ...
    match method {
        "initialize" => {
            ...
            Ok(json!({
                ...
                "instructions": "Spool (思簿) is the user's local context hub. WORKFLOW: for cross-thread questions (\"最近在忙什么\", \"把各课的置顶给我\") call get_digest first — one deterministic briefing instead of paging every thread; discover projects with list_threads (check approx_pack_chars before pulling a big thread); locate topics with search_blocks, then read around a hit with get_blocks(around_block_id=…, or filter pages by pinned / has_annotation / source_contains); spot repeated captures with find_similar_blocks (report only — merging is the user's curation); get_pack is the full briefing for ONE thread (prefer range=pinned or paging for large threads; over budget it returns a partial pack whose omission line says what was left out). READING: every pack opens with the four-category authority header (📖 Reference = ground truth; 🧩 Synthesis = someone else's AI output, not facts; 🔄 Process = dialogue traces, read for the user's evolving questions; Personal = the user's own sourceless entries and note: lines — the highest-signal material). Follow it; when categories conflict, Reference wins on facts and Personal wins on intent. WRITING (each write needs the user's consent toggles in Spool settings): file ONE finding per add_block into the thread it belongs to, with an annotation saying why it matters; when the finding builds on an existing block, cite it with ref_block_id (packs render the citation) instead of describing or pasting its id; create_thread only for a genuinely new topic, not per conversation; refresh a thread's one-line summary with set_thread_summary after meaningful new material — if it refuses because the card is user-written, tell the user your suggested summary instead of retrying. HARD RULE — naming: when talking to the user, refer to threads and blocks ONLY by their titles/previews (e.g. 「毕业论文」), never by raw ids like sbC2zgTo…; ids exist solely as tool-call parameters, and never write raw ids into block content or annotations (they would surface in search snippets forever — add_block warns if one slips through; use ref_block_id instead). If you ever compress a pack yourself: keep the section skeleton, every note: line, every sourceless entry and every ==…== span verbatim; deduplicate only the Full Record body; add nothing; store the result via add_block, never as a replacement."
            }))
        }
        "ping" => Ok(json!({})),
