...
**v2 charter (designed 2026-07-08 — design only, NOTHING below is built; implementation waits for Ocean's per-item go).** The loop v1 leaves open: the user meets something in chat, and the model has no way to find *which thread it belongs to* short of pulling full packs one by one. v2 closes retrieval, and draws the curation line explicitly.

- **IN (proposed minimal set):**
  ...
  3. Resources probe, behind the same toggle: `resources/list` exposes threads as `spool://thread/<id>` (title as name, summary as description) and `resources/read` returns the pack text; emit listChanged after MCP writes. Ship **only if** a Claude Desktop smoke test shows @-mention actually surfaces them usefully; otherwise the current empty `resources/list` answer stays.
