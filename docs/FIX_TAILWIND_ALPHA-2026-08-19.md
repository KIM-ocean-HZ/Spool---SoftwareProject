...
> **分支 `fix/tailwind-alpha`，从 `main`（`5189c7b`）开的，没推、没合。**
> 起因写在 `docs/HANDOFF-2026-08-19.md §2`，Ocean 的原话是
...
它是代码本来的意图，但发布版从来没长成那样，所以这是**观感上的新东西**。
说「要」就合；说「不要」，就把那两个 `<aside>` 上的 `bg-paper-2/40` 删掉再合——
不要靠继续让 Tailwind 丢规则来达到「不要」，那样另外 118 处也一起还回去了。

其余三条（弹窗罩子、淡边框、hover）都是代码原意，且都很淡，默认按「合」处理。
