...
⚠️ **当天装了两次**:12:24 装了全部;**12:38 又装了一次**,只为返工日期提醒那个窗口(§2-bis)。
第二次没有 schema 改动,库仍然是 v17。

**装机核对过的**(08-13 12:24 首装):真库 `PRAGMA user_version` **14 → 17**,`integrity_check` ok,
...
- ⚠️⚠️ **提醒是一张时刻表,不是一个窗口**:`NOTICE_STAGES = [60, 30, 7]` ——
  **两个月前 / 一个月前 / 一周前各提醒一次**(Ocean 第四轮:「两个月,一个月,一周,差不多这样吧」)。
  最多 3 行,多的折成「还有 N 个」。已经过去的不提(他库里全是 —— 每条来源行都带着抓取那天)。
  ⚠️ **这个数改过三次,历史别丢**(全过程在 §2-bis):7 天 → 拿真库一跑**一条都不显示**
  → 不设期限、常驻最近 3 条 → 他看过实物给了 30 → 再问一句之后给了这张时刻表。
  **前三个值都是我在桌上拍的,这一个不是。**
- ⚠️ **✕ 只压掉「当前这一档」,不是永久删除,也不是压一周固定时长**。
  实现上**没动 schema**:`date_dismissals` 本来就有 `created_at`,拿它反推出
  「他点 ✕ 的时候这个日子在哪一档」(`noticeStage`),日子跨进更近的一档就自动回来。
  ⚠️ 写入必须是 `DO UPDATE SET created_at`,**不能是 `DO NOTHING`** ——
  第二次点 ✕ 要能记下新的那一档,否则那个键看起来是坏的。
- ⚠️ **它不在窗口顶上,在块流里面**(他第三轮:「不要固定在顶部,需要可以跟随 blocks 滑动」)。
  挂在 `BlockFeed` 的第一行、滚动容器**里面**,所以会跟着块一起滚走。
  挂在 `ThreadView` 那一层就是固定的 —— 别搬回去。
- ⚠️ **颜色是暖色 + 半透明**(他第三轮:「弹窗颜色不行,做一个暖色,透明一点的」):
  `--notice-warm` / `--notice-warm-edge` 两个新 token,琥珀色低透明度铺在纸色上,
  **不是填充色块**,块流仍然是主角。
...
## 2-bis. ⭐ 那个 7 天是怎么被抓出来的(台账 §3.14,值得照抄的做法)

日期提醒第一版设了「只提今天 ~ 7 天内」。装上去之后 Ocean 一句「**项目顶上那条日期提醒,
没有看到这个东西**」。

**没有去猜,也没有去改 React** —— 写了一个**临时探针**(一个 `.test.ts`,只读打开他的真库,
把每个块的正文喂给真正的 `findDates`,打印每个日期离今天几天、会不会显示),跑完**立刻删掉**。
一屏输出就把话说死了:

- 识别器**没坏**:31 个日期全找出来了,Cornell / MIT / CMU / USC 的截止日期一个不漏
- **他最近的一个日子是 23 天后**,而这功能真正要管的申请截止日期在 **114–170 天后**
- 所以 7 天那个窗口下,**正确的行为就是什么都不显示**,而且要到 8/25 才会有第一条

⚠️ **能装下 114 天的窗口和没有窗口没区别**,所以拍板的结果是**把窗口整个删掉**,
改用「最近 3 条 + ✕」来兜底。**旋钮换成了规则,这和 §13.2 折叠那件事是同一个动作。**

⚠️ **这条方法本身要留下来**:UI 的事验不了(合成点击驱动不了 webview),
但**「它凭什么会显示」这一层是纯函数,永远可以拿真库跑一遍**。
下次再出「我没看到」这类反馈,先跑探针,别先改代码。

**第三轮(同一天):他看过「不设期限」的实物之后**,给了三条:
「还是在一个月前再弹出提醒吧,然后一个星期再问一次」「弹窗颜色不行,做一个暖色,透明一点的」
「不要固定在顶部,需要可以跟随 blocks 滑动」。三条都落地了(§1.6)。

**第四轮(同一天):** 我问了一句「一个月对申请这种事是不是太晚」,他答
**「两个月,一个月,一周,差不多这样吧」** —— 这把「一个窗口」换成了**一张时刻表**,
也顺带定死了 ✕ 的语义(压掉当前这一档,跨进下一档自动回来)。已落地。

⚠️⚠️ **60/30/7 这张表下他现在实际能看到什么,已经拿探针核过(2026-08-09 那天),下一窗别再猜**:

| 项目 | 现在显示 | 下一条什么时候醒 |
|---|---|---|
| 〈Flux〉 | **1 条**(9/1 CMU,23 天后,落在 30 档) | 2026/10/02(12/1 那条进 60 档) |
| 〈申请规划〉 | **0 条** | **2026/10/16**(12/15 那条进 60 档) |

⚠️ **他打开〈申请规划〉仍然是空的,要到 10 月中才有第一条。** 这是时刻表的必然结果,
**已经明确告诉过他两次**。如果他又说「没看到」,**先问他开的是哪个项目**,
再跑一次 §2-bis 那个探针 —— **别上来就改代码。**
