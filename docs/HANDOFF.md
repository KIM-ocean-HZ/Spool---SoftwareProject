...
**MCP 三个新 prompt 已实现并提交;两份实机评审报告已分诊;Ocean 的批复已记下。
下一窗照 §2.7 动手,先读 H。**(2026-08-04)工作区干净。
**Ocean 2026-08-04 明示:先不推 main。** 推之前必须再问一次。
...
3. **两份实机评审报告(ChatGPT 5/10、Claude 6.5/10)逐条验过并分诊** —— 11 条真 bug、
   2 条假问题(已实测证伪)、1 条根子在我的种子数据(**已修**)、7 条要 Ocean 拍板。
   **Ocean 的批复已逐条记进 §2.7 H,那是下一窗的起点**。除种子修复外一行代码没改
   (Ocean 明示执行留给下一窗)。

上一窗的两件事(网页工程债 B `7fa8fe7` 已上线、`/Applications/Spool.app` 换成 main 构建)
仍然有效,细节在 §2.1–2.2 与 §3。

**👉 下一窗:先读 §2.7 的 H(Ocean 的批复,含两条要先出设计稿的),再按 G 的顺序动手。**
其中 H-6(distill 必填参数)和 B 类都不需要再拍板,可以直接开工。
...
| B | 🚩 **按实机评审报告修** —— 报告已分诊完毕,**Ocean 2026-08-04 的批复也已记下**(可见编号的想法、prompt 工具化、PDF 全文搜索、先不推 main、零摩擦引导、distill 必填参数)。**下一窗的主线** | §2.7,先读 H |
...
#### H. 🚩 Ocean 2026-08-04 的批复与新要求(**下一窗按这个走**)

**H-1. 「AI 把内部编号写进正文」怎么办 —— Ocean 提了第三条路,值得走**

> 原话:「没有办法把内部编号映射到用户可以看见的数据上吗,比如给每个 block 一个独立的编号,
> 或者点击高亮显示该 block,从 UI 界面来接受 id。」

**审查结论:这条比原来的「拦 or 不拦」都好,它把矛盾解掉了。**
现在的硬规则是「id 只是工具参数,永远别给用户看」,理由是裸 nanoid 对人是噪声。
但**需求是真的**:AI 要指着某一块说话,用户要能找到它。现在只能靠引用预览,而库里有
一模一样的重复块时,预览根本指不准(实验室里就有三条同样的话)。给块一个**人能看的号**,
AI 就再也没有理由去写裸 id —— 于是 add_block 改成硬拒绝不再有代价。**顺序应该是:
先做可见编号,再把写入改成硬拒。**

两条落地路线,建议下一窗出设计稿给 Ocean 选:
- **便宜路线(几乎零成本,建议先做)**:pack 里每一块本来就打着 `[2026-06-29 10:00]`,
  App 的块流里也看得见时间。把「时间戳 + 项目标题」正式确立为**对用户的指代方式**,
  写进 instructions 和工具描述(现在只说"别说 id",没说"那该怎么指")。
- **正路(要动 schema)**:`blocks` 加一个**每个项目内单调递增、永不复用**的序号
  (`#12`),App 块流里安静地显示,pack 与工具返回都带上;点一下能跳到那一块。
  ⚠️ 派生序号(按时间排第几)不行 —— 删一块后面全变,不稳定。必须落库。
  这条要 schema v9 + 迁移 + 两个渲染器(`assemble.ts`/`templates.ts` + `mcp.rs` golden)+ GUI,
  **和 H-3 的 FTS 改动是同一次迁移,应该合并成一轮做**(硬规则 5、真库备份)。

**H-2. 三个新 prompt 做成工具 —— 批准,做。**
函数已经写好(`weekly_review_prompt_text` / `thread_health_prompt_text` / `distill_prompt_text`
+ `thread_health_report`),多挂一个工具调用面即可。注意工具版的语义:返回的是"材料 + 指令",
所以工具描述要写清"调完之后请按返回里的指示做",别让模型把指令当数据。

**H-3. 存进来的 PDF 里的字要能搜到 —— 批准,做。**
`blocks_fts` 现在只索引 content + annotation,附件抽取正文完全在外面(实测讲义里的
句子 `total = 0`)。要动 schema(v9)+ 迁移 + 重建索引。**和 H-1 的正路合并成一轮。**
⚠️ 动 schema 前备份真库(硬规则 3、memory `spool-db-wipe-incident`)。

**H-4. 先不推 main。** 本地已攒 5 个提交,工作区干净。**推之前必须再问一次。**

**H-5. 🚩 MCP 使用引导必须"清晰且零摩擦" —— 这是新的产品要求,不是 bug 修复**

> Ocean 原话(照抄,别改写):「MCP 端做不到 prompt 提示,只有 claude code 可以,
> 但是主要 MCP 还是 GPT、claude desktop 这些,所以 MCP 的使用 prompt 指引需要更加清晰
> 且零摩擦:①不能让用户自己在网页和 APP 中零散地去拼凑 MCP 使用方式;②不能让用户
> 自己去问 AI『spool 能干什么』;③不能让用户使用 MCP 之后自己都不清楚 AI 在调用什么工具
> (因为用户看到的是 prompt 名称,而用户不一定知道 prompt 是做什么的)。」

这条**推翻了 `DESIGN_NEXT_STAGE.md` §4.2 的前提**——那里写的是「用户在 Claude Desktop
斜杠菜单直接看到 → 零学习成本的功能发现面」,实测 Claude Desktop 和 ChatGPT 都不露 prompts。
已在 §4 长期计划加了第 7 条。**要先出设计稿给 Ocean 批,再动手(硬规则 6)。** 方向建议:

| 对应哪一条 | 做法 |
|---|---|
| ②不用问「能干什么」 | 服务器的 `initialize` instructions 是**每个客户端都会读的唯一一处**。在开头加一小段「用户可以直接这样说」的三句话示例(「我最近在忙什么」「机器学习课我卡在哪」「把这条存回去」),模型自然会在第一轮就告诉用户能干什么 |
| ①不用自己拼凑 | 用法**只维护一处**。设置里已有的「复制使用提示」升级成一段可直接粘的开场白,官网/教程/README 全部指向它,不要三处各写一份(现在教程种子里那句还停在「一键接入 Claude Desktop / Cursor」,见 §5.8,一并修) |
| ③看得见 AI 在调什么 | **工具返回文本的第一行用大白话说"我刚做了什么"**(例:「读了〈机器学习课〉的完整简报,共 17 块,其中置顶 3 块」)。用户在客户端里只看得到工具名 + 一坨 JSON,这一行是他唯一能读懂的东西。成本很低,收益直接命中这一条 |
| 兜底 | H-2 的工具化本身就是这条要求的一半:工具是模型顺着用户的大白话主动调的,不需要用户知道"prompt"是什么 |

**H-6. Claude Code 上 distill 点不动 —— 已查准,不是坏了,是"必填参数"卡住**

Ocean 实测:
- 点菜单里的 `/spool_lab:distill` → `Error: Missing required argument: project. Usage: /mcp__spool_lab__distill project range`
- 手打 `/spool_lab:distill` → `Unknown command`

诊断(已核实):
1. `distill` 和 `thread_health` 的 `project` 声明成 `required: true`,**Claude Code 在客户端侧就拦下了**,请求根本没发到服务器。要跑得打 `/mcp__spool_lab__distill 机器学习课`。
2. `/spool_lab:distill` 只是菜单里的**显示名**,真正的命令名是 `/mcp__spool_lab__distill`。
3. 对照组:`weekly_review` 没有必填参数,**点一下就能跑** —— 这就是证据。

**修法(下一窗)**:把 `project` 改成非必填;缺参数时服务器不报错,而是返回一句
「你要哪一个项目?」+ 现有项目清单,让对话接着走。这样菜单里点一下就有反应,
零摩擦(正好是 H-5 要的)。**顺带把 `compress_pack` 的 `thread_id` 一起改。**
...
| 7 | 🆕 **MCP 零摩擦使用引导**(Ocean 2026-08-04 提出):不许用户零散拼凑用法、不许用户自己问「spool 能干什么」、不许用户不知道 AI 在调什么工具 | **未开工,要先出设计稿批复**。起因:实测主力客户端都不暴露 prompts,§4.2「斜杠菜单即发现面」的前提不成立 | 方向与三条要求的对应表在 §2.7 H-5 |
