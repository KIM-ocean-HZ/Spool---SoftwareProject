...
> 完成后删除本文件。
>
> **本窗要做的第一件事:§1 那份给 Ocean 的真手指测试单 —— 代码已写完并过了四关,
> 只差这一步。做完之后才是 §2 的 MCP 生态宣传。**

---

## 0. 一句话状态

**26 个提交在本地 main 未推送**。基线全绿:`npx tsc -b` / `npx vitest run`(152)/
`cargo test`(16)。真库 `integrity_check ok`,v8 / 15 块,本窗一个字节没动
(mtime 停在 11:31,改造全程用隔离 identifier)。

**浮窗挪进独立进程已经做完了**(`74b87f1`)。设计稿 §5 的四个关卡过了三个半:
...
## 1. 👉 本窗任务:把测试单交给 Ocean,按结果收尾

代码写完了,自动化能验的都验了(§3)。剩下的**只能真手指**:AX 抢焦点那条路要辅助功能
授权,隔离构建拿不到;给它授权又会把 tap 推到 HID 层、合成事件当场消失 ——
两头堵死,这是 memory `isolated-verify-workflow` §14 记过的老限制。

**所以流程是**:先换装(需 Ocean 明示,§4)→ 把下面的测试单**用大白话**交给他 →
按他的反馈收尾。写测试单前读 memory `write-plainly-for-ocean`(他说过「你写的我没看懂」)。

### 1.1 要他验的两组

**第一组:主窗到底还沉不沉底(这次改动的全部目的)**

摆好窗口:比如后→前是 `VSCode - Spool主窗 - 文本编辑`。然后在**文本编辑**里
⌘C 复制一段字、双击 ⌥。逐条看:

1. 浮窗弹出来的那一刻,Spool 主窗有没有动?(**应该纹丝不动** —— 以前会沉到最底下)
2. 用鼠标点一下浮窗里的 ✕、图钉、批注框,主窗有没有动?(**应该还是不动**)
3. 关掉浮窗之后,窗口顺序是不是还是原来那样?

**第二组:note-first 那五条 + 四条关闭路径,原样重跑一遍**(§5.1 有清单,
和 `a3cfba2` 那次验的是同一批,这次是回归)。

### 1.2 顺带让他看一眼的两处

- **活动监视器里会多一个 Spool 进程**(叫 `spool --overlay`),这是设计里认过的,
  不是 bug。实测常驻 87MB。
- **Dock 里不应该多出第二个 Spool 图标**(辅助进程是 Accessory 策略)。
