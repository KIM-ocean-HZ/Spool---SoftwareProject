...
- ⚠️⚠️ **提前量:30 天(`NOTICE_WINDOW_DAYS`)**,最多 3 行,多的折成「还有 N 个」。
  已经过去的不提(没什么可做的了,而且他库里全是 —— 每条来源行都带着抓取那天)。
  ⚠️ **这个数改过两次,历史别丢**(全过程在 §2-bis):7 天 → 拿真库一跑**一条都不显示**
  → 改成不设期限、常驻最近 3 条 → 他装完看了实物,说**「还是在一个月前再弹出提醒吧」**。
  **30 是他看过两个极端之后给的数,不是拍出来的。**
- ⚠️ **✕ 是「先收起一周」,不是永久删除**(他第三轮加的:「然后一个星期再问一次」)。
  实现上**没动 schema** —— `date_dismissals` 本来就有 `created_at`,读的时候只认
  7 天内的那些(`SNOOZE_DAYS`)。⚠️ 写入必须是 `DO UPDATE SET created_at`,
  **不能是 `DO NOTHING`**:第二次点 ✕ 要能重新起算一周,否则那个键看起来是坏的。
...
⚠️⚠️ **30 天窗口下他现在能看到什么,已经拿探针核过,下一窗别再猜**:

| 项目 | 现在显示 | 下一条什么时候出现 |
|---|---|---|
| 〈Flux〉 | **1 条**(9/1 CMU,23 天后) | 2026/11/01(12/1 那条) |
| 〈申请规划〉 | **0 条** | **2026/11/15**(12/15 那条) |

⚠️ **他打开〈申请规划〉会是空的,一直空到 11 月中。** 这是 30 天窗口的必然结果,
**已经明确告诉过他**。如果他又说「没看到」,先确认他开的是哪个项目 —— 别又去改代码。
