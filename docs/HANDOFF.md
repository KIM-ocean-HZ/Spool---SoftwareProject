...
| **2** | **请他看一眼更正那一摊改成什么样了**(§1-ter)。⚠️ 要他自己去〈申请规划〉找那条 08-10 更正过的块 —— 那是真库里唯一一处 | 不用,直接说 |
| **3** | 把 §0.2 那张「等你点头」的清单念给他,让他一次挑完 | — |
| **4** | **按他 08-10 定的顺序开工:case-study 四、五、六期**(A3) | — |
...
| 序 | 事情 | 为什么现在能做 | 全稿 / 位置 |
|---|---|---|---|
| **A3** | ⭐ **case-study 四、五、六期(先做这个)** | 闸门已开。台账已到 §3.21,素材是够的 | `DESIGN_CASE_STUDY.md`,§4-12 |
| **A0** | **官网精修** | ⚠️ **Ocean 08-10:「官网现在的内容已经落后最新版本软件很多了,需要重新设计功能展示和介绍,甚至交互小组件也需要重新设计。」** 是整页重做,不是换几张图 | memory `next-stage-goals-website-portfolio`(08-10 那一节) |
| **A1** | **截图全套重建** | 放在官网之后 | §5.1 + memory(**多场景铁律**) |
| **A2** | **演示视频** | 同上。⚠️ **截图和视频一起做完,才动官网 Hero 那一屏** | §5.1 / §5.2 的 F、G |
| **A5** | **首日价值二期**(线轴 + 三个计数) | 全新一稿,Ocean 08-10 提的,**未开工**。⚠️ 开工前先把 §4 那四个拍板问完 | ⭐ `DESIGN_FIRST_DAY_VALUE.md` |
...
| **2** | **推不推本窗这个提交** | 本窗提交的是更正返工 + v21 + 二期设计稿 + 交接。⚠️ 上一批(到 `33d9802`)08-10 晚**已经推了** |
...
- **本窗那个**(更正返工 + v21 + 二期设计稿 + 台账 §3.21 + 交接)—— ⚠️ **没推**
