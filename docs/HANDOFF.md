...
以及**把 A5 排到 case-study 和官网精修之间**(§0.1)。⚠️ **没提交、没装机。**
...
| **圆圈徽章第二轮(居中 + 尺寸)** | ⚠️ **没提交** | ⚠️ **没装** —— 他现在机器上的还是上一轮那个偏上的圆圈 |
...
| **2** | **请他看一眼更正那一摊改成什么样了**(§1-ter)。⚠️ 要他自己去〈申请规划〉找那条 08-10 更正过的块 —— 那是真库里唯一一处。⚠️⚠️ **但圆圈徽章第二轮(§1-quater)还没装机** —— 他现在看到的圆圈仍是偏上的那一版,**要看新的得先装** | 不用,直接说 |
...
| **2** | **推不推那两个提交** | 一个是更正返工 + v21 + 二期设计稿 + 交接(已提交、没推);⭐ **另一个是圆圈第二轮(§1-quater),现在连提交都还没做**。⚠️ 上一批(到 `33d9802`)08-10 晚**已经推了** |
| **2-bis** | ⭐ **圆圈第二轮要不要装机** | 纯前端一行 CSS,**schema 没动 → 不迁移、不用重开 AI 客户端**(判据见 §6.2-quinquies 第 7 条)。**代价只有一次装机的几分钟**;不装的话他下次看到的圆圈还是偏上的那一版 |
...
5. ⚠️ **他提的圆圈那两条(数字没居中、圈太大)已经改好了,但还在代码里没装机**(§1-quater)。
   **要他看见就得先装一次**,纯前端、不迁移、不用重开 AI 客户端。**装不装问他**(§0.2 第 2-bis)。
...
- ➕ ⚠️ **2026-08-10 晚又变了三处(更正那一摊)**:**块头的 `#1` 全部变成了圆圈徽章**
  ...
  ➕ ⚠️ **圆圈又改过一轮(§1-quater),而且要装机才看得见**:现在是 **13px 的圈 / 9px 的数字**,
  比第一轮小了一圈。**拍之前先确认装的是带这一轮的那个版本**,否则拍出来的是偏上、偏大的旧圈。
...
- ⭐ **本窗那个(圆圈第二轮 + 台账 §3.22 + 交接)—— ⚠️ 连提交都没做**,
  改动是 `src/components/ThreadView/SeqBadge.tsx` 一行 class + `docs/` 两个文件
