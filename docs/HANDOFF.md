...
| 序 | 做什么 | 全稿 | 备注 |
|---|---|---|---|
| **1** | ⚠️ **问 Ocean:块流看着对不对** | 本文件 §2 | **含 MD 渲染的版本已经装好了**(08-12 11:13)。**块正文的样子这一窗变化最大**(标题、列表、粗体、代码块),而这一层只有他能验 —— 他还没回话之前,别在这一摊上继续改 |
...
- **08-12 装了两次**:第一次(10:38)给他看上一窗那四屏;第二次(11:13)**含 §11.4 + §10.1**,
  就是现在 `/Applications` 里跑着的那一份。**两次都核对过库没变**(v14 / 35 块 / 3 项目),
  CPU 落回 0.0%、`spool.db-shm` 当场重建。
- ⚠️ **他对块流(MD 渲染)的反馈还没有** —— 见 §2。
- 装法见 §6.2-quinquies。旧版备份留在 `/Applications/Spool.app.previous-*`(现在有三份),
  库备份在桌面 `spool-snapshot-20260808-pre-reinstall*.db`。
  ⚠️ **都是 Ocean 机器上的东西,要清先问他。**
...
**08-12:两窗的改动已经合成一个提交**(`42b470d`,33 个文件),Ocean 明示要提交的。
⚠️ **还没推。** 自检 `git log -1 --pretty=full | grep -iE 'claude|anthropic|co-authored'` = clean。
