...
⚠️ 一行代码都还没装到他机器上,这是下一窗第一件事。**

| 摊 | 代码 | 装到 `/Applications` 了吗 |
|---|---|---|
| MD 排版(08-12) | ✅ | ✅ 装了(08-12 10:23)—— 但**标题字号 08-13 又改了**,他看到的还是旧的 |
| 项目文件库 v15(08-12) | ✅ | ❌ **没装** |
| 08-13 这一窗全部(v16 / v17 在内) | ✅ 四条基线全绿 | ❌ **没装** |

### 0.0 ⭐⭐ 开工顺序

| 序 | 做什么 | 备注 |
|---|---|---|
| **1** | ⚠️⚠️ **问 Ocean:能不能装** | **这一次装是 v14 → v17,一次走三步迁移**,其中 **v15 会毁东西**(删 `attachments.block_id` 列、删 `kind='url'` 的行)。他库里 `attachments` 一行都没有,**所以实际删不掉任何东西** —— 但按 `spool-db-wipe-incident` 的规矩必须他点头。装法见 §6.2-quinquies,**多一步:装完要告诉他去重开 AI 客户端**(§3) |
| **2** | 装完请他看四处新东西 | 都是只有他能判的:①标题字号现在对不对(§2)②长块现在还折不折(§2)③项目顶上那条日期提醒(§5-3)④块流右上角那个「只看我写的」按钮(§4-7) |
...
装完之后,**正在跑的那些 `--mcp` 子进程还是老二进制**,它们连的库却已经是 v17 了。
`EXPECTED_SCHEMA_VERSION` 的守卫会让它们**明确报错**(不是静默出错),但对 Ocean 来说
表现就是「AI 那边突然连不上了」。

→ **装完要用大白话告诉他:把 ChatGPT / Claude 那边的客户端完全退出再打开一次。**
...
**08-13:这一窗和 08-12 那一窗的改动都还没提交。** 上一次提交是 `42494ae`(docs)。
...
### 6.9 ⚠️ 真库现在长什么样(2026-08-12 量的)

**`PRAGMA user_version = 14`,2 个工作区 / 7 个项目 / 34 块,`attachments` 0 行。**

⚠️ **`attachments` 是空的,这件事很重要**:它是 v15 迁移「删 url 附件」这条
在他机器上**删不掉任何东西**的原因(`DESIGN_PROJECT_FILES` §5.1 ③ 已查证过一次,08-12 复核仍然成立)。

⚠️ **装完之后它会变成 v17。** 三步迁移里只有 v15 那步会删东西,v16/v17 一个是加列、
一个是建空表。
