...
**v0.3.0 已公开发布**,官网下载按钮现在**直接下 dmg**(不再中转 Releases 页)。
干净安装冒烟测试已过。本机回到「新用户」状态,历史数据归档在桌面,截图临时环境已拆。
基线全绿:`npx tsc -b` / `npx vitest run`(152)。

**下一个窗口的主线换了:不是 Windows,是「上手体验」四件事**(§2)。
Ocean 2026-07-30 晚明确:**Windows 版排到最后**,先把新用户第一眼的体验做对。

---

## 1. ⚠️ 待 Ocean 动手

1. 【你自己用 app 的前提】**去授权 Input Monitoring**。冒烟测试实测:
   Accessibility 已授,**Input Monitoring 没授**。没有它,双击 ⌥ 只在 Spool 自己在前台
   时有效——日常场景(在别的 app 里 ⌘C 再双击)不工作。
   系统设置 → 隐私与安全性 → 输入监听 → 勾 Spool → **托盘退出 → 重开**。
   ⚠️ 这个「半授权」状态还有个副作用,会抢掉 Claude Desktop 的双击 ⌥,见 §5.3。
2. 【历史数据要不要装回】桌面 `Spool-Data-Archive-2026-07-30/`,`README.txt` 有恢复步骤。
   现在 app 里是全新空库 + 教程脉络。废纸篓里还有第二份保险,确认无误后可清空。
3. 【重启 Claude Desktop】`spool-demo` 条目已删,重启才生效。
4. 【一个小澄清,挡着 §4.2】「先让 **Claude Code** 达到完美」——指的是
   **Claude Code(命令行)** 还是 **Claude Desktop**?现在 app 的一键接入只支持
   Claude Desktop 与 Cursor 两个目标,**Claude Code 是第三种配置方式**
   (`claude mcp add` / `~/.claude.json` / 项目级 `.mcp.json`),没做。
   要不要把它加成第一等公民,决定了 §4.2 的第一步。
5. 【演示视频】`docs/DEMO_SCRIPT.md` 分镜已写好,你实机录。
   Ocean 已定:**录英文版**——所以最好等 §2 的英文教程落地后再录,否则镜头里
   还是中文教程脉络。

---

## 2. 下一个窗口的主线:新用户上手体验(Ocean 2026-07-30 晚追加)

四件事里第 4 件已经做完(§5.1),剩下三件。**它们互相咬着,建议按下面顺序做,
不要单独拆散**——尤其 2.2 和 2.3 必须同批上线。
...
**所以这不是纯 bug,是个死胡同 UI**。两条路,建议 A:

- **A(建议,小改)**:`granted-later` 态给一个「**立即重启 Spool**」按钮,
  点了直接 `app.restart()`(Tauri v2 有这个 API),重启后 preflight 为真 →
  首帧就是 `hidden`,提示条自然消失。死胡同变成一键。
- **B(要先验证可行性)**:授权翻转时**在同进程内重建 tap**,免重启。
  风险:macOS 对 Input Monitoring 的 TCC 决定通常按进程缓存,
  新建 tap 未必拿得到事件。**动手前先写个探针验证**,别直接改产品代码。
...
**验收**:干净安装(删数据目录)在英文系统上首启 → 英文 UI + 英文教程;
中文系统上首启 → 中文 UI + 中文教程;手动切过语言的用户重启后不被覆盖。
...
Ocean 要:收集用户邮箱,发新版本时自动推送,**用户可以不订阅**;另外要一个收集反馈的入口。

**⚠️ 动手前必须先解决一个叙事冲突(这是设计决策,得 Ocean 拍)**:
Spool 现在对外承诺的是「**无需账号 · 不做任何追踪 · 断网可用 · 本体零出网**」——
官网 fineprint、README、发布说明、隐私页全都这么写,而且 CSP 的 `connect-src`
只放行 Tauri IPC(memory `mcp-first-pivot`)。
**把邮箱收集做进 app 里 = 亲手推翻这个卖点**,还得给 app 开出网权限。

**我的建议**:
- **放官网,不放 app**。官网本来就是联网的,收邮箱不动摇 app 的隐私叙事。
  app 里最多放一个「查看新版本」的外链按钮(点了开浏览器),不内建检查更新的出网。
- 反馈入口同理:官网一个表单 / 一个 GitHub Issues 链接;app 里放外链。
- 若 Ocean 坚持要 app 内订阅,那必须**同步改口**:官网/README/发布说明里
  「不做任何追踪」「本体零出网」的措辞都要重写,而且要在设置里做成明确的 opt-in。
  这不是加个输入框的事,是产品定位的事。
- 技术上官网是 GitHub Pages 静态站(`.github/workflows/pages.yml` 自动部署),
  **收表单需要第三方**(邮件列表服务或表单服务)——引第三方依赖按硬规则 2 要 Ocean 批。
...
**建议修法**:把 SUPPRESS 也绑到 Input Monitoring 上——gate 不生效就不该独占。
小改动,但要配实机验证。可以和 §2.1 一批做(同一个模块)。
...
⚠️ 那份批复里「默认语言仍是 zh」的前提**已被 Ocean 推翻**(见 §2.3):
默认要改英文。等 §2.2/2.3 落地、界面和教程都有英文版之后,
**双语截图这件事要重新评估**(那时 EN 截图就名正言顺了)。
