...
#### C. 我自己欠的一笔:实验室种子数据是错的

**「埋了什么」表里写的两处「裸 id 泄漏」,实际是 19 位和 20 位** —— 真 id 是 21 位。
检测器按规格工作,两份报告却都据此下了"检测器漏报"的结论(一份判 P1、一份判 P0)。
**下一窗第一件事:把种子里的假 id 换成真的 21 位混合大小写串,重跑一遍确认 check_library 能报出来。**
教训:埋坑的数据必须先自测能被抓到,否则测出来的是假问题、还会带偏评审。
...
1. **第一批(不需拍板)**:C 的种子修复 → B-1(附件正文纳入预算 + 拒绝文案不再推荐刚失败的路)
