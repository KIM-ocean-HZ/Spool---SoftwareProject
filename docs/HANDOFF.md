...
✅ **已提交(`0578622`)、已推送、已装机(08-09 23:44)。** Ocean 当窗三样一起点的头。

| 摊 | 代码 | 装到 `/Applications` 了吗 |
|---|---|---|
| 上一窗四件(文件库三期 / AI 可读开关 / 分流 / 读改 brief) | ✅ | ✅ **装了**(08-09 15:57) |
| 这一窗五件(A/B/C/D/E 意图路由) | ✅ | ✅ **装了**(08-09 23:44) |

**装机核对过的**:schema **不涉及迁移**(库 v19 / `client.ts` 19 / `mcp.rs` 19,三处早就一致);
装完真库仍是 **v19 / 46 块 / 2 个活项目 / 1 个附件**,`integrity_check` ok。
三个旁证齐:CPU 稳定 0.0%、`spool.db-wal`/`-shm` 当场重建、进程起来了。
装完又拿**装好的那个二进制**对着真库跑了一次:**`tools/list` = 18**、
以前缺席的四个工具(`check_library`/`get_follow_up_brief`/`request_file_access`/
`suggest_follow_up_brief`)现在 instructions 里全点到了、中文触发词在、
`list_threads` 报出了 `following_up`/`files`/`files_locked`。
备份:`~/Desktop/spool-snapshot-20260809-234225-pre-routing.db`(v19 / 46 块,验过)。

### 0.0 ⭐⭐ 开工顺序

| 序 | 做什么 | 备注 |
|---|---|---|
| **1** | ⚠️ **确认他重开过 AI 客户端了** | 装完那一刻还有 **4 个 `--mcp` 子进程**跑着老二进制,新工具在那些连接里不会出现。schema 没变,所以**不会报错,只会"少一个工具"** —— 比报错更难发现。**这边验证不了,只能问他** |
| **2** | ⚠️⚠️ **提醒他:跑验收第 1 句之前,先去把那个 PDF 的 ✓ 点掉** | 真库里**唯一那个附件 `ai_access` 已经是 1**(他 08-09 自己批的),所以「有哪些文件、里面写了什么」**不会触发申请** —— 模型直接就读到了。不点掉 ✓,第 1 句验的是个假阳性 |
| **3** | ⚠️ **请他跑 §7 那五句** | ⭐ **判据是事件流里的工具名,不是模型说了什么**(§6.2-ter 第 6 条) |
| **4** | ⚠️ **请他看一眼窗口** | §6.2-bis:没有任何自动化能验「长得对不对」。上一窗那四件的界面(待审面两张新卡、右侧栏 ✓、原文「N 字」)**至今没人看过** |
| **5** | 开工 F | §0.1 —— 这一摊只剩它了 |
...
| 事情 | 全稿 |
|---|---|
| ⭐⭐ **F:块的三个出处字段(schema v20)** —— 意图路由那一稿**唯一剩下的一件**,也是唯一动 schema、唯一要重生 golden 的重活。**建议单独一窗**,别和别的混。⭐ **开工清单在下面 §0.2** | `DESIGN_MCP_INTENT_ROUTING.md` §4.6 + §8.3 |
| **首日价值三小项** | §4-7 第三条:捕捉满三条 → 一行安静提示;「今天读了什么」日卡。**这是「产品下一程」那条里最后剩的一件** |
| **M4 定时跟进** | ⚠️ 仍然卡着两个前提(§4-8) |
| **case-study 四、五、六期** | 卡在「代码全做完之后」 |
| **截图 + 演示视频全套重建** | 同上,§5.1 |

### 0.2 ⭐⭐ F 的开工清单(schema **v20**,给下一窗直接照做)

**Ocean 08-09 拍板乙,而且是在看过「这要动 golden」的代价之后选的 —— 别替他省。**
全稿 `DESIGN_MCP_INTENT_ROUTING.md` §4.6,落地时还要补 §8.3 那三件欠账。

**要加的三列**(都可空,**不属于**「会毁东西的迁移」那一类,§6.3-9):
...
| 序 | 做什么 | ⚠️ |
|---|---|---|
| 1 | schema **三处一起动**:`client.ts` 的 `SCHEMA_VERSION`、`mcp.rs` 的 `EXPECTED_SCHEMA_VERSION`、`client.test.ts` 那一堆 `toBe(n)` | §6.3-5。⚠️ `downgradeToVn` 是**链式**的,要在**最前面**加 `downgradeToV19` 并让原来的头先调它(现在的头是 `downgradeToV18`) |
| 2 | 写侧:`add_block` 和 `propose_blocks` 的 item 各开这三个参数 | ⚠️ **用户在界面上不填**,这一版 UI 只**读**。加输入框等于给主路径加负担 |
| 3 | **两侧渲染器一起动**(`assemble.ts` + `mcp.rs` 的 `render_*`),块头下面多一行 `↗ 网址 · 查于 X · Y 之前该复查` | ⚠️ **这是这一摊唯一允许动 `render_project_file` 那一层的时候** |
| 4 | **重生 golden**:`TZ=Europe/London GOLDEN_WRITE=1 npx vitest run src/lib/pack/assemble.test.ts` | ⚠️ §6.5,**必须带 TZ**;能合并成一次就合并 |
| 5 | `source` 的描述改成「只留短标签」,URL 从此进 `source_url` | ⚠️ 真库里那 20 个长标签**不回改**(只增不改) |
| 6 | 补 `get_project_overview` 的 `needs_attention.due_for_recheck` | ⚠️ 代码里**留了注释**说它属于 F;这是 `recheck_after` 的**兑现口之一** |
| 7 | pack 里过期的那条,块头标一句 | ⚠️ **不隐藏、不作废**,只是标一句 |
| 8 | 加工具/加参数的收尾三连 | `annotations` + **`OPENERS` 加行** + 真跑。⚠️ 加参数不加工具的话,`every_tool_is_reachable_from_the_routing_text` 不会红,但**该说的话还是要说** |

⚠️⚠️ **`recheck_after` 一定要有兑现口(第 6、7 步),否则它就是一列没人读的数据** ——
§5-3 那条「v13 给了作废/更正两把刀,但没有任何东西提醒用户去用」就是这么来的。
⏸ **UI 上的复查提醒这一窗不做**:项目顶上那条日期提醒(v17,`NOTICE_STAGES`)是
**另一套机制**(它从正文里认日期),两套合并要单独想,别顺手做。

⚠️ **F 之前先把 §7 的五句验收跑完。** F 一进来 golden 就重生了,
「A–E 一次没重生」这条判据从此失效 —— 混着做,出问题分不清是谁弄的。
...
✅ **代码已装机(08-09 23:44),`tools/list` 对着真库确认是 18 个。**
⚠️⚠️ **但要先让 Ocean 完全退出再打开 AI 客户端** —— 装完那一刻还有 4 个 `--mcp`
子进程跑着老二进制。schema 没变,所以它们**不会报错,只会少一个工具**,比报错难发现。
