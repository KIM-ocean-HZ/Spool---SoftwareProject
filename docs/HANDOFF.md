...
⚠️ **同日深夜又发生了两件事,它们改了下一窗的主线,先读这两条:**

1. **Ocean 用过右侧栏之后给了第二轮反馈**,总判断是
   「**目前的 ui 被无用的按钮堆砌满了**」。全部归位在
   **`DESIGN_WORKBENCH.md` §9** —— **那是下一窗的主线**。
   ⚠️ §9.5 记了**两处跟已拍板项的冲突**(项目内周回顾、⋯ 菜单入口),动手前先解决。
2. **新增设计稿 `DESIGN_CONTEXT_HYGIENE.md`**(上下文卫生:长度 + 过时),
   含一轮前沿调研。**Ocean 明示它排在 §9 那一摊之后**,别抢在前面做。

⚠️ **深夜这一窗没有写任何代码**,只动了三份文档(见 §6.6)。
...
## 1. Ocean 08-06 拍的板(决策留档 —— 晚上一轮 §1.1,深夜一轮 §1.2)
...
### 1.2 深夜那一轮:第二轮 UI 反馈 + 上下文卫生三块板

**UI 那半边**(总判断「目前的 ui 被无用的按钮堆砌满了」)全部归位在
`DESIGN_WORKBENCH.md` **§9**,不在这里复述 —— **§3 的清单直接指过去**。

**上下文卫生那半边**:背景是他提出「引入联网搜索、提炼之后,信息入口越来越多,
没用的干扰信息也越来越多;是否有最有效率的方式保存上下文」。
全稿 `DESIGN_CONTEXT_HYGIENE.md`,**当场拍了三块板**:

1. ✅ **剪贴板 pack 的读法表头做成开关**(默认极简),**不删掉**。
   ➕ **表头本身要重审重写** —— 他说「目前表头是开发初期的作品」。
   ⚠️ 它是 golden 锁步的,而且**任何 locale 下保持英文**(§6.4 两个受众)。
   ⚠️ **改措辞可以,改「四类」要非常慎重** —— 已经两次判过「不开第五类」。
   **数字支持他**:golden fixture 那份 pack 4,329 字符,表头占 **2,616(60%)**。
2. ✅ **顺序:先 W7(批注当标题),再做「取代」。**
3. ✅ **「取代」要支持「没有替代者、只标记不作数」。**
   ➕ 他提的那个问题(替代信息常常只是大段里的一句话,要不要复制其余内容)
   **逼出了正确形状:不复制**。见那份稿子 §3.1.1 —— 分两档,
   「部分更正」那一档**旧块一个字不动、照常全文进 pack**。

### 1.3 深夜第二轮:两条,**已落地并提交**

1. ✅ **不新增「项目内周回顾」** —— 它跟 `distill` 是同一件事
   (`DESIGN_WORKBENCH.md` §9.5)。
2. ✅ **动作改名**(「让所有功能变得精明扼要」),**已落地**:
   **提炼结论→压缩 / 整理去重→去重 / 找找新进展→跟进 / 生成周回顾→周回顾**
   (第四个是照他这条原则顺手做的,**他没单独点名,可以推翻**)。
   ⚠️ **只改了标签。** MCP 工具名、提示词名、`engine_runs.action` 取值**一个字没动**
   —— 那是别的客户端读的契约。留档 `DESIGN_WORKBENCH.md` §9.7。
   ⚠️⚠️ **他同时问了「压缩应该把所有块都压一遍吗,还是只留一块?」——
   答案和它带来的硬约束在 §9.8,下一窗动这块之前必须读。**
   一句话:**都不是。压缩 = 一条结论 + 一份「取代了哪几块」的清单**,
   所以**「压缩」这个名字今天只兑现了一半**,要等 H2(取代关系)落地。
   ⚠️ 两条硬约束:**pin 过的块、有批注的块一律不许压**(那是产品的长处);
   **只压过程,不压事实**。
...
⚠️ **主线变了(Ocean 08-06 深夜明示):不是接着跑阶段三,是
`DESIGN_WORKBENCH.md` §9 那一整套右侧栏重构。**
他亲手用过之后的总判断是「**目前的 ui 被无用的按钮堆砌满了**」。
**动手前先读 §9.5 那两处冲突。**

### 第一梯队 —— 右侧栏重构(`DESIGN_WORKBENCH.md` §9)

| # | 事情 | 状态 |
|---|---|---|
| **R1** | 右侧栏**展开时比左侧栏窄** | ✅ 可开工,**最便宜的一条**。「默认收起」**已经是了**(`railCollapsed: true`);⚠️ **宽度反了** —— `DEFAULT_RAIL_WIDTH = 320` > `DEFAULT_SIDEBAR_WIDTH = 280`,`MIN_RAIL_WIDTH = 260` > 侧栏的 200。改 `layout.ts` 两个常量 |
| **R2** | 「AI 活动」并进右侧栏,和待过目同一格 | ✅ 可开工。⚠️ **别把「待你过目 N 条」那一格的常驻显示一起改掉**(§3.3 拍过:0 条时写「没有待过目的」) |
| **R3** | 主视图 ⋯ 菜单收敛成三个:**打包 / 捕捉 / 完成项目** | ✅ 可开工。⚠️ **这条推翻了 §3.2 的「保留 ⋯ 菜单入口」** —— 以 Ocean 亲手用过的判断为准 |
| **R5** | 设置页把 **AI 引擎拆成单独一项**,不和 MCP 挤 | ✅ 可开工。§3.5 只做了一半(动作搬走了,页面没拆) |
| **R4** | **右侧栏整体重构**(四个组件 + 项目管理区) | 主体工程,细节全在 §9.3 / §9.4。⚠️ 主轴是「内容占主要面积,控制项收起来」 |
| **W3-c** | **模型 / effort 选择** | ✅ 可开工,**被他的反馈从"可做"抬成"必做"**(原话:切换模型「根本没得选」,要能选 opus/sonnet/haiku)。两个 CLI 都有 `--model`;codex 的 `model_reasoning_effort` 已验证是真键。⚠️ **模型名要真机确认,别硬编码猜的** |
| **W4** | **流式进度** | ⚠️⚠️ **它从"排最后"变成了"UI 的主角"**(他要 VSCode 插件那种正在打字的效果),**但风险一个字没变**:claude 要换 `--output-format stream-json`,**会动到现在唯一验证过能跑通的解析路径**。**必须真机跑一次**(§6.2-ter),并且**做成能回退的** —— 解析失败退回今天的「攒完再读」,别让一次运行整个失败 |

### 第二梯队 —— 上下文卫生(`DESIGN_CONTEXT_HYGIENE.md`)

⚠️ **Ocean 明示:整摊排在第一梯队之后。别抢跑。**

| # | 事情 | 状态 |
|---|---|---|
| **W7** | **批注当标题** | ✅ 已采纳,**这一摊的第一件**。理由见 `DESIGN_WORKBENCH.md` §7:**多数块没有批注**,所以「有批注的才当标题」,没批注的一个字不动。⚠️ 动块流要先读 §6.5 的 golden fixture 规矩 |
| **H2** | **取代关系**(schema **v13**) | 拍板过了,形状在 `DESIGN_CONTEXT_HYGIENE.md` §3.1.2。两处:`blocks.stale_at` + `ref_kind`(cites / supersedes / corrects)。⚠️ **只有用户能提,AI 不许**。⚠️ **「压缩」这个新名字在等它** —— 见 `DESIGN_WORKBENCH.md` §9.8 |
| **H3** | **剪贴板 pack 极简化 + 表头重写** | 拍板过了(§1.2 第 1 条)。⚠️ golden 三侧 + 表头保持英文 |
| **H4** | **超预算时降级成目录**(而不是直接扔掉老块) | 未拍板细节,形状在 §3.3 |
| **H5** | **AI 一句话标签** | ⚠️ **排最后是有理由的** —— 做完 W7/H2/H4 很可能就不缺它了(调研证据见那份稿子 §2.5) |
...
| **E3** | **第三个引擎档(Gemini CLI)** | ✅ 可开工。Ocean 明确点名了 gemini。照 §7.2 的规矩,`DESIGN_AI_ENGINE.md` §7.3 那张表要长出第三列 |
...
⚠️ **一条跨梯队的工程提醒**:W7 / H2 / H3 / H4 **全都动 pack 或块流渲染**,
...
10. ⚠️ **新增(08-06 晚):工作台** —— 全稿 `DESIGN_WORKBENCH.md`。四阶段:
    ...
    ~~三 自动化 + 周回顾独立~~ ✅(模型选择还没做)/ **四 流式进度**
    ⚠️ **新增(08-06 深夜):阶段五 —— 右侧栏重构**,`DESIGN_WORKBENCH.md` **§9**。
    Ocean 亲手用过之后的第二轮反馈,总判断「**ui 被无用的按钮堆砌满了**」。
    **这是下一窗的主线**,阶段三剩下的模型选择和阶段四的流式都被并进它
11. ⚠️ **新增(08-06 深夜):上下文卫生** —— 全稿 `DESIGN_CONTEXT_HYGIENE.md`。
    治两个病:**上下文太长** 和 **信息过时**。含一轮前沿调研(2026-08 查,会过期)。
    五件:W7 批注当标题 → 取代关系(v13)→ pack 极简化 + 表头重写 →
    超预算目录降级 → AI 一句话标签。
    ⚠️ **Ocean 明示:整摊排在第 10 条的阶段五之后。**
...
⚠️ **08-06 深夜补:第 3、4 条现在有归宿了,别再当孤立的旧账看**
- 第 3 条(截止日期没人管)→ `DESIGN_WORKBENCH.md` §9.4 的项目管理区按 DDL 排序,
  `threads.deadline` 这一列一直在。⚠️ **但别顺手做**,那是另一件事
- 第 4 条(重复块想清清不动)→ `DESIGN_CONTEXT_HYGIENE.md` §3.1 的取代关系。
  ⚠️ 这条稿子里有一句要记住:**缺的不是删除权限,是「从发现到动手」之间那一步**,
  而取代给了它一条不用删除就能走的路
...
⚠️ **上面那句「本窗没有提交」已经过期,别照着它判断。**
右侧栏那一整摊**已经提交了**(`fd32014` feat(workbench) + `3c81318` / `3862e0c` 两条 docs),
工作区当时是干净的。

**08-06 深夜这一窗:三份文档 + 一次改名,已提交,工作区干净。**
改名碰了 11 个源文件,但**每一处都只是那四个词**,没有别的改动。
基线在提交前跑满三条:`tsc` 干净 / vitest **210** / cargo **39**。
