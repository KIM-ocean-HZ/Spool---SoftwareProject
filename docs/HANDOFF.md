...
| ~~3~~ | ~~codex 最后一格(V2)~~ | `DESIGN_AI_ENGINE` §7.6-bis | ✅ **08-10 也真跑通了**,结掉两件半 —— 见 §2.6 |
| **1** | ⚠️ **Ocean 亲眼看一眼新 UI** | 本文件 §3 | **这一窗没能截到图**(原因在 §3),右侧栏和项目管理都换了样子 |
| **2** | **挑一件「顺序未定」的开工** | 下表 | 我的建议:**`DESIGN_WORKBENCH` §10**(块正文 MD 渲染 + 动作条减到 7 键)—— 纯前端、不依赖任何额度、Ocean 最常看的那一屏 |
...
### 2.6 ✅ codex 最后一格(V2)也收了 —— 但有一件是「拿不到」

**额度恢复,真跑了一次 distill,exit 0。全文 `DESIGN_AI_ENGINE` §7.6-bis。**

| V2 三件 | 结果 |
|---|---|
| 花费字段 | ✅ **有 token,没有美元。** 已接上(`parse_codex_usage`),卡片显示 token + 「花费未知」。⚠️ **绝不写 0**,那会被读成「这次免费」 |
| 流式事件名 | ✅ 成功路径四个名字拿到了。⚠️ **工具调用那一格仍然没见过**(这次它没调工具) |
| 模型目录 | ❌ **拿不到** —— 四个事件里没有任何地方说跑的是哪个模型。**所以 codex 的模型选择器继续不做**,理由从「没验过」升级成「验过了,拿不到」 |

⚠️ **顺带第三次证实了写入闸那件事**:codex 也是一块没写(32→32),
最后一句是「如果你同意这块结论,我再将它追加回…」。**claude / gemini / codex 三个引擎
在这里行为完全一致** —— 是提示词的设计,不是哪个引擎的毛病(§2.3)。
...
| ~~V2~~ | ~~codex 那条路的最后一格~~ | ✅ **08-10 真跑通了**,见 §2.6。⚠️ 只剩「工具调用事件的形状」没见过 |
...
cargo test --manifest-path src-tauri/Cargo.toml   # 53 通过(2026-08-10,E3 +6、codex 花费 +2)
