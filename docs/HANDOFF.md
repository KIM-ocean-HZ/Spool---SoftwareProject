...
> **08-10 以后的日期都是真的**(`644fba3` / `63f2824` / `e5163b8` 全部 2026-08-10)。
...
**08-10 这一窗做完了五件:F 装机 + 推送(真库已升 v20)、五句验收里的四句取证、
首日价值三小项落地并提交(「产品下一程」到此全清)、对外材料的工具面数字订正、
台账补两条(§3.19 / §3.20)。**

⭐⭐ **app 代码到此真的全清了** —— 所有「排在代码全做完之后」的闸门(截图 / 演示视频 /
case-study 四五六期)**现在都开着**。
...
| **首日价值三小项** | ✅ `e5163b8`(Ocean 08-10 明示提交) | ❌ **没装** |
| **数字订正 + 台账 + 交接** | ✅ 见 §6.6 | —(纯文档) |
...
| 序 | 做什么 | 要不要 Ocean 点头 |
|---|---|---|
| **1** | ⚠️⚠️ **第一句话就提醒他:完全退出再打开 AI 客户端**。真库已是 v20,他那边还连着的老 `--mcp` 子进程是 v19 的 —— 不重开,**每次工具调用都报错**(原话见 §0.3 第 3 条) | 不用,直接说 |
| **2** | ⚠️⚠️ **请他看一眼左边栏那张新卡**。⚠️ **他八成看不见** —— 卡只在当天有捕捉时出现,得先让他捕一条(§2-bis) | 不用,直接说 |
...
| 序 | 事情 | 为什么现在能做 | 全稿 / 位置 |
|---|---|---|---|
| **A1** | **截图全套重建** | ⭐ app 代码全清了,Ocean 早就批过「排在代码全做完之后」 | §5.1 + memory `next-stage-goals-website-portfolio`(**多场景铁律**) |
| **A2** | **演示视频** | 同上。⚠️ **顺序是 Ocean 定的:截图和视频一起做完,才动官网 Hero 那一屏** | §5.1 / §5.2 的 F、G |
| **A3** | **case-study 四、五、六期** | ⭐ 闸门已开。台账已到 §3.20,素材是够的 | `DESIGN_CASE_STUDY.md`,§4-12 |
| **A4** | **首日价值装机** | 代码已提交(`e5163b8`),没装 | §6.2-quinquies |

**B 类 —— 只欠一次真跑,几分钟的事**

| 序 | 事情 | 卡在哪 |
|---|---|---|
| **B1** | **§7 第 5 句(`corrects`)** | 一次都没发过。真库 49 块 `ref_kind` 全是空的 —— 这条路在真客户端上零次 |
| **B2** | **§7 第 6 句(F 的出处三字段)** | F 已装机,前提没了,随时能跑 |
...
| # | 要他定什么 | 现状 / 取舍 |
|---|---|---|
| **1** | **首日价值要不要装机** | 代码已提交没装。⚠️ 这次装机**不会迁移**(schema 仍是 v20,纯前端改动),所以比 F 那次简单得多 —— 但**仍要按 §6.2-quinquies 备份真库** |
| **2** | **推不推这几个文档提交** | `e5163b8` 之后还有一个纯文档提交(台账 + 交接 + 数字订正)。⚠️ 官网 `site/story.html` 改了,**推到 GitHub ≠ 上线** —— spoolapp.org 要单独部署,那是另一次明示 |
| **3** | **A 类三件先做哪个** | A1 截图 / A2 视频 / A3 case-study。⚠️ **A2 依赖 A1**(Ocean 定的顺序),A3 独立 |
| **4** | **B 类那三句真跑,现在跑还是等** | 每句几分钟,他自己在 ChatGPT 里发一句就行(大白话版在 §7.1) |
| **5** | **库目录里那批备份能不能清** | ⚠️ **是用户数据,一份都别自己删**。清单在 §6.9,现在又多了两份 |
| **6** | **`/Applications/Spool.app.previous-20260810-161112` 能不能删** | 那是 v19 的旧 app。他确认 v20 用着没问题就能删 |

### 0.3 ✅ F 的装机做完了 —— 这一节改成留档,下次遇到迁移照这个来
...
2. ⏳ **§7 第 5 句(`corrects`)一次都没发过。** 真库 49 块的 `ref_kind` **全是 NULL**,
   这条路在真客户端上仍然是零次。
3. ⏳ **§7 第 6 句(F)也还没跑** —— F 已经装机了,前提没了,随时能跑。
4. ⏳ 上上窗那四件的「好不好看」仍然没人验过(装了但没请他看)。

### 2-bis ⚠️⚠️ 首日价值这一摊,Ocean 今天多半**看不见**

**左边栏那张日卡只在「今天有捕捉」时出现。** 真库里的捕捉一共 5 条,**全在 2026-08-05**。
所以他现在打开 app,左边栏和以前**一模一样** —— 这是对的,不是坏了。

**要让他看见,得先让他捕一条**:在任何 app 里选一段字 → ⌘C → 快速双击 ⌥。
捕完左边栏顶上就该冒出「今天读了 1 条」。
...
2. ⚠️⚠️ **v20 已经装上去了(08-10 16:11),他还没被提醒重开 AI 客户端。**
   不重开,ChatGPT 那边**每一次**调 Spool 都会回「数据库比这个 MCP 服务还新」。
   **完全退出 ChatGPT 再打开就好,配置一个字都不用改**(§0.3 第 3 条)。
3. **想看首日价值那张新卡,得先随便捕一条**(§2-bis)—— 他今天没捕过东西,卡是不出现的。
...
7. **产品下一程剩下的三条**(原 `DESIGN_NEXT_STAGE.md` §4.3–§4.5,那份稿子已归档)
   ...
   - ~~**「我的思考」凸显**~~ —— ✅ **两半都落地了**:W7(批注当标题)08-07,
     **「只看我写的」过滤 2026-08-13**
   - ~~**首日价值**:捕捉满三条 → 一行安静提示;「今天读了什么」日卡~~ ——
     ✅ **2026-08-10 落地(§1),连稿子里第三条「README + 教程讲透 MCP 没配也全功能」一起**。
     ✅ 已提交 `e5163b8`,⚠️ **未装机、未推送**。**⭐ 这一整条(产品下一程)到此全清。**
...
- `63f2824` 交接同步 —— ✅ **已推送**(`origin/main` 到此)
- `e5163b8` 首日价值三小项 + README 数字订正 —— ⚠️ **没推**
- (还有一个纯文档提交在它后面:台账 §3.19/§3.20 + 交接重写 + case-study/官网数字)—— ⚠️ **没推**

⚠️ **推送要单独问 Ocean。** 而且 ⚠️ **推到 GitHub ≠ 官网上线** —— `site/story.html` 改了,
spoolapp.org 是单独部署的,那是**另一次**明示。
...
**08-10 16:11 装完 v20 之后现场量的**:`PRAGMA user_version = 20`,**49 块**,
2 个活项目,1 个附件(`ai_access = 1`),`integrity_check` ok。
⚠️ **下次要准数就现场量,别抄这个。**

**49 块是怎么来的**:08-09 是 46,08-10 验收第 3 句的分流批准后 +3
(一个原文块进〈申请规划〉,两个分流块分别进〈申请规划〉和〈Flux〉,后两个都 `ref_block_id`
指着原文块)。⚠️ **全库 `ref_kind` 仍然全是 NULL,`stale_at` 一条都没有** ——
作废和更正这两把刀在真库上**一次都没被用过**(§5-3 那条毒的现场证据)。

⚠️ **备份文件又多了两份**(都是 08-10 装机产生的,**都是用户数据,别自己删**):
`~/Desktop/spool-snapshot-20260810-pre-v20-install.db` 和库目录里的
`spool.pre-migration-v19-2026-08-10T08-11-22-533Z.db`。
⚠️ **`/Applications/Spool.app.previous-20260810-161112` 也还在** —— 那是上一版 app(v19 二进制)。
Ocean 确认 v20 用着没问题之后可以删,**但要问他**。
...
⭐ **下一窗只要请他跑第 5 句和第 6 句** —— 1–4 句 08-10 已经跑完并全过(§7.2)。
