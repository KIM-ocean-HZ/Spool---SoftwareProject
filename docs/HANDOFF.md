...
**Ocean 的七条 UIUX 返工全部落地(`DESIGN_WORKBENCH.md` §9.13)。四个引擎动作在新建的
测试库上真跑过。⏸ 还没换装 —— 等 Ocean 明示。**
...
上一份交接把 `DESIGN_CONTEXT_HYGIENE`(上下文卫生)定成第一梯队。**Ocean 明示先不做**:

> 「先不做上下文卫生,UIUX 仍存在巨大问题,设计有缺陷,**禁止按照目前的思路做下去**。」

那一摊没取消,只是排在 UI 过关之后。**别一上来就去做 W7。**

### 0.2 ⏸ 还没换装,并且 Ocean 还没看过新界面

代码提交了(`ac79c90`),**没推、没换装**。
我只截到了「普通项目 + 新右侧栏」那一屏(见 §3.1),**项目管理那一屏没截到** ——
合成点击驱动不了这个 webview(§6.2-bis),要看它只能装上去自己点左侧栏置顶的「项目管理」。
...
### 3.1 ⚠️ 第一件:让 Ocean 看新界面,拿到第二轮反馈

我只截到了「普通项目 + 新右侧栏」这一屏(1280×840,**没白屏**):
...
| # | 事情 | 取舍 |
|---|---|---|
| **A** | 右侧栏 210px 会不会太窄 | 截图上「跑一次,结果留在这里等你过目。」这种句子换行比较碎。窄是他要的(「把位置阔给工作区」),但 210 是我挑的数,他没看过 |
| **B** | 模型选择器里的 `opus` 怎么办 | 它现在是坏的(§2.2)。删掉 = 少一个选项;留着 = 用户选了会失败 |
...
| **上下文卫生** | `DESIGN_CONTEXT_HYGIENE.md` 那一摊 | ⏸ **被 Ocean 明示往后推**(§0.1)。UI 过关之后再说 |
...
11. ⚠️ **上下文卫生** —— 全稿 `DESIGN_CONTEXT_HYGIENE.md`。
    ...
    超预算目录降级 → AI 一句话标签。
    ⚠️ **08-07 晚:被 Ocean 明示往后推**(§0.1),UI 过关之后再说。
...
**08-07 晚这一窗:代码 + 文档已提交(`ac79c90`),工作区干净。没推。**
