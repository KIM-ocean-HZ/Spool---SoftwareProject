...
### 3.44 The cost model was an estimate, and the pack's own first line is what will decide it (2026-08-20)

Form C shipped as far as it can ship without a paid key: `spool-ai`, a **separate crate**
producing a separate binary, is now the only part of Spool that opens a socket. The property
that made form C acceptable over form A is mechanically checkable rather than promised —
`cargo tree -e normal` on the main crate returns nothing for `ureq|rustls|reqwest|hyper|
native-tls|openssl|ring|curl`, so the shipped `spool` executable links no HTTP or TLS stack at
all. Putting the dependency in the main crate and merely not calling it would have downgraded
that from a structural property to an intention, which is the exact reason WORKPLAN §6.2
rejected form A.

**Measured, 2026-08-20:**

- End to end, with a deliberately invalid key: Spool's Rust side resolves the sidecar, spawns
  it, hands the request (key included) over stdin, and gets back
  `{"ok":false,"kind":"auth","status":401}`. `cargo test --lib the_whole_chain_answers --
  --ignored` reproduces it. It costs nothing to run, because a rejected key is rejected before
  any tokens are billed.
- Release sidecar: **1.27 MB** on `aarch64-apple-darwin` (`opt-level="z"`, LTO, stripped).
...
* **The saving in the cost model has a load-bearing assumption nobody had looked at.** §6.2's
  ...
  being byte-identical run to run. The pack's own opening paragraph is `Generated by Spool on
  <date>. N blocks total.` — so the prefix breaks at roughly the thirtieth token whenever the
  day changes or a block is added, and adding a block is precisely why anyone re-compresses.
  What can cache is the instruction block, and that is why it is the system message and the
  pack is the user message. §6.2's "表头和旧块几乎必然命中" is the claim most likely to be
  wrong, and `usage.cached_input_tokens` is the probe that will settle it.
* **"The endpoint did not report cache hits" and "nothing hit the cache" are different
  results, and only one of them is a measurement.** The field is `Option<u64>` the whole way
  from the sidecar to the dialog, and when it is absent the cost is computed as if everything
  missed and labelled an upper bound. Rendering unknown as zero would have made the first real
  run produce a number that looked measured and was invented — on the one figure this project
  intends to publish.
* **Asking the thing under review to report its own score is not review.** The compressed
  briefing's block counts are counted by Spool from both texts; the model is asked only for a
  sentence about what kinds of thing it merged, and that sentence is displayed separately and
  labelled as its own claim. When the model omits it, the dialog says it omitted it.
* **A side-by-side view of 50,000 characters is not a check, it is a chore.** The rules require
  `note:` lines, sourceless entries, `==spans==` and whole sections to survive verbatim, so the
  dialog counts each of those in both texts and names what is missing before the user reads a
  line. The diff shows where; the audit line says whether there is a where worth looking for.
* **A credential's storage decision was already made in this repository, in the opposite
  direction.** `settingsStore.ts` actively scrubs `groqKey` / `geminiKey` out of `settings.json`
  on every launch, and `settings.json` is documented as hand-editable and travelling between
  machines. So the new key goes to a 0600 file of its own, outside the settings file and outside
