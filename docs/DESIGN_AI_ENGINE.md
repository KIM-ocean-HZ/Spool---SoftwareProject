...
> 状态:**待 Ocean 批复**。前提:2026-07-29 Ocean 已批 §4.1 升主线;
> 本稿是动手前的细化(硬规则 6)。目标版本 v0.4.0。
...
- **M1**:检测 + 设置小节 + 单动作「提炼结论」端到端(含取消/超时)——先证明管道。
- **M2**:补齐三动作 + 队列串行 + toast/错误细化;§4.2 prompts 面同批(同源常量)。
- **M3**:与 §4.3「AI 活动」折叠区汇合,形成"动作 → 痕迹"闭环。
