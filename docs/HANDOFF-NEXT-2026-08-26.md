...
| ① | 刻度旁的小 block 预览 | 鼠标靠上去才展开（Ocean 选的），当前块居中、上下各 5 个命中、越远越小，点一下跳过去。刻度仍然 16px |
...
| ⑨ | 专注时间一直累积 | 休息只清倒计时，总专注时间接着往上加 |

⛔ **三处没照字面做**，都写在 `TEN-ITEMS-2026-08-27.md` §3：⑨ 写的是「已专注」不是「今天已专注」
（本次开机以来的数，写「今天」会是假的）；③ 的双击**没有**跟着长按一起等半秒（会拖慢捕捉，改用
可取消的定时器做互斥）；⑥ 的文案是错的所以改了。

状态：TS 测试 **738** 全绿（新增 26 个），`npm run build` 通过。**已换装**（2026-08-27 20:45）——
授权没掉（`installed at HID/active`），真库四个计数和正文/批注总字节前后完全相同，回执在那份 §6。
