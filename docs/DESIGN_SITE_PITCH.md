# 设计稿:官网怎么讲才有人肯下载、肯学 — 已全批(2026-08-02)

> **Ocean 2026-08-02 批复:「三档全做。」** 第一、二、三档的页面改动已在提交 `8e92776` 落地,
> 逐块与文案见 §3/§4。**两处与本稿的出入,都记在这里:**
> 1. **§4.5 那两张对比图没有拍成截图**,改用官网现成的 `.mock` 示意图语言(和「02 · 留」那块
>    同一套),并在图注里**明写「这是示意图,不是截图」**。理由:「第六周 21 条」的内容无论如何
>    都得是编的,套上截图的外壳等于用更权威的形式讲同一段虚构;示意图零素材成本、也不会误导。
>    要真截图随时可以补,得先造一个「第六周」状态的演示库。
> 2. **§6 的对外动作(注册表挂号、Show HN 等)一件都没做** —— 硬规则 7,要你单独明示。
>
> 还没做的:**§6 末尾提的「装完之后头 60 秒」审计**,以及**推 main 部署官网**(要你明示)。
