# 设计稿:把 MCP 生态讲清楚(官网 + app 内)— 已全批并落地(2026-08-02)

> 2026-08-02 撰写,对应 HANDOFF §2(Ocean 2026-07-31 提出)。两层目的:对外用**已支持的
> 客户端阵容**拉动下载,对内**别让用户看不懂**。
>
> 商标那部分是会过期的外部事实,**2026-08-02 逐家查了官方页面**,来源列在 §8。
>
> **Ocean 2026-08-02 批复:「纯文字名字 + 我们自己画的中性图标;教程种子预留到后面一起做。」**
> 即拍板点 1 走 **A**,拍板点 2/3 按建议,拍板点 4 **本轮不做**。已在提交 `8e92776` 落地:
> - 官网 `#mcp` 段内加阵容块(两组卡片 + 名字标签),**零第三方 logo、零新增资源文件**;
> - 设置页折叠段补「在哪儿说」一行,`VS Code` 改成 `Visual Studio Code`;
> - 顺带修掉一个改动前就存在的窄屏 bug(见 §7 末)。
