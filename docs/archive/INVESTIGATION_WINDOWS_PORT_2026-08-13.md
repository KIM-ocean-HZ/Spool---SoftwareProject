...
> ✅ **2026-08-15 Ocean 拍板，三条：**
>
> 1. **双击手势先不做** —— 改为另做一个新的、方便的手势；
> 2. **先不签名分发**；
> 3. **首版范围接受**本报告的建议：**能装能跑 + 快捷键捕捉 + 打包 + MCP 接得上**。
>
> 因此本报告 §0 建议顺序里的**第 6 步（双击 Alt）整条移出首版**，
> **签名安装包那一段降为后续**。其余分阶段面仍然有效，
> 尤其两个编译阻断项仍在（`capture.rs:757` 的 `RESTORE_FOCUS_APP` 未 cfg 门控；
> `mcp.rs` 无条件调用 `localtime_r` / `gmtime_r` / `timegm` / `mktime`）。
> 当前任务面见 `docs/BACKLOG-2026-08-15.md` §3.2。
