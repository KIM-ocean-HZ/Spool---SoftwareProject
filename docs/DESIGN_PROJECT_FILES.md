...
| 期 | 内容 | 能不能独立发 |
|---|---|---|
| **一** | schema v14 + 迁移 + 右侧栏「项目文件」+ 删块上那两个键 | ✅ 能。**这一期用户就已经拿到 Ocean 要的全部界面变化** |
...
**③ 删 url → 已经存在的 url 附件必须有个去处,不能静默消失。**

⚠️ **迁移里要有一条明确处置**,三选一(**未定,开工前问 Ocean**):
(a) 迁成对应块正文末尾的一行链接文字;(b) 迁进项目文件库,当成一种只有链接没有文件的条目;
(c) 直接删。**⚠️ (c) 是删用户数据,除非他明说,否则不做。**
