...
⚠️⚠️ **覆盖失败是静默的** —— §3 那条 `codesign -dvv` 必须**亲眼看到 Authority 第一行是
Developer ID**，不是 `Spool Dev`。

**发布之后还有两件收尾，别漏**（交接 §0.3）：

1. ⚠️ **当场存下公证回执** —— `xcrun notarytool submit --wait` 的完整输出（submission id +
